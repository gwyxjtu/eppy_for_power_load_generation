...
#iddfile = "D:/EnergyPlus88/Energy+.idd"
...
buildings = {
    ...
    'IDC':"1ZoneDataCenter",
    "IDC2":"2ZoneDataCenterHVAC_wEconomizer"
...
def LoadCal(building, area, weather,name,f):
    # 初始化idf
    ...
    print(keys[0:3],keys[3])
    rv = rv.apply(lambda x: x * ra/J if x.name in keys[0:3] else x)  # 面积等比缩放
    rv = rv.apply(lambda x: x *ra /1000 if x.name == keys[3] else x)#光照千瓦变瓦
    rv.to_csv("res1124/"+name+'_'+f+"_IDC"+'.csv', index=None)#生成csv文件
    return rv


if __name__ == '__main__':
    main_dic = get_dict("./weathers/1124/")
    ...
    for c in prov:
        for f in list(main_dic[c].keys()):
            ...
            print(filename)
            try:
                load = LoadCal(buildings["OfficeMedium"], 14000, filename,c,f)
            except:
                ans.append(c)
                print("error "+c)
