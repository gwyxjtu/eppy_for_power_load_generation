...
def LoadCal(building, area, weather,name):
    ...
    rv["Electricity Load [kwh]"] = rv[[a, b]].apply(lambda x: (x[a] + x[b]), axis=1)
    # logP = a + ' + ' + b#验证a和b是否是light和equipment
    rv.rename(columns={'Heating:EnergyTransfer [J](Hourly)': 'Heating Load [kwh]'}, inplace=True)
    rv.rename(columns={'Cooling:EnergyTransfer [J](Hourly)': 'Cooling Load [kwh]'}, inplace=True)
    rv = rv[keys]  # 只要时间和3种负荷数据
    #rv = rv.apply(lambda x: x * ra/J if x.name in keys[0:3] else x)  # 面积等比缩放
    rv = rv.apply(lambda x: x /J if x.name in keys[0:3] else x)
    rv = rv.apply(lambda x: x /1000 if x.name == keys[3] else x)# 光照千瓦变瓦
    rv.to_csv("res1124/"+weather+'.csv', index=None)#生成csv文件
    return rv


if __name__ == '__main__':
    main_dic = get_dict(epwdir)
    ...
    prov = list(main_dic.keys())
    for c in prov:
        for f in list(main_dic[c].keys()):
            filename=main_dic[c].get(f)
            print(filename)
            try:
                load = LoadCal(buildings["OfficeMedium"], 1, filename,c)
            except:
                ans.append(c)
                print("error "+province)
